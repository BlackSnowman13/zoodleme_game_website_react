...
export function JigsawGamePage({
    category, setCategory, state, setState, image, setImage, pieceNumber
} : {
  ...
  useEffect(() => {
	...
	return (
      ...
      <img id="dragImage" className="hidden absolute w-0 h-0 cursor-move pointer-events-none" />

      <div id="infoParent" className="hidden absolute w-[100%] h-[100%] justify-center items-center -z-10">
        <div className="flex w-[95%] h-[95%] justify-end">
          <div id="infoDisplay" className="flex flex-col items-center bg-[#FFC6AA] transition-all ease-in rounded-full border-[#FF7A37] border-[5px] w-[50px] h-[50px]">
			...
			<div className="flex flex-col justify-between h-full w-full">
				<div className="h-fit w-full flex justify-around bg-[#FFAE86] pt-5 pb-5">
					<span id="back_button" className="material-symbols-rounded text-[40px]" style={{fontSize: '40px'}}>arrow_back</span>
					<span id="show_image" className="material-symbols-rounded text-[40px]" style={{fontSize: '40px'}}>image</span>
          <img id="edge" src={"/assets/edge-icon.svg"} alt="icon" className="w-7" />
					<span id="hint" className="material-symbols-rounded text-[40px]" style={{fontSize: '40px'}}>emoji_objects</span>
					<span id="refresh" className="material-symbols-rounded text-[40px]" style={{fontSize: '40px'}}>cached</span>
					<span id="info" className="material-symbols-rounded text-[40px] z-10" style={{fontSize: '40px'}}>info</span>
				</div>
      ...
      <div id="completionMenu" className="hidden absolute bg-[#ff7a373f] h-full w-full justify-center items-center top-0">
          <div className="flex h-3/4 w-[340px] bg-gradient-to-t from-[#FEB793] via-[#FFDDCC] to-[#FFFEFE] border-[5px] border-[#FF7A37] justify-between flex-col items-center">
              ...
              <div className="flex justify-center items-center aspect-square w-[80%]">
                  <Image id="completionImg" className="aspect-square w-[100%] rounded-[5px] border-[#f8d0b8] border-[5px]"  src={image} alt="completionImage" width={1000} height={1000} />
              </div>
